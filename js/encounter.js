...
const renderEncounters = function(){
    ...
    db.collection('encounters').get().then(data =>{
...
const createEncounter = function(encounters, filters){
   const filteredEncounters = $.grep(encounters, element =>{
        return element.symptom.toLowerCase().includes(filters.searchText.toLowerCase());
    })
    $("#tbody").empty();
    filteredEncounters.forEach(element =>{
        ...
        let tr = $("<tr><td>"+ element.id + "</td>" + "<td>" + element.symptom  + "</td>" + "<td>" + element.prescription  + "</td>" + "<td>" + element.admitted + "<td>" + element.date + "</td> </tr>");
        tr.append(buttonElement);
        $("#encounterTable").find("tBody").append(tr);
    })
}

const deleteEncounter = function(element){

    db.collection('encounters').doc(element.id).delete().then(()=>{
        alert('Patient record deleted successfully!')
        const encounterIndex = encounters.findIndex(encounter => encounter.id === element.id)
    if(encounterIndex != 1){
...
$('#saveEncounter').click((event)=>{
    ...
    db.collection('encounters').doc(id).set(encounter).then(()=>{
